...
)
{
	...
	/* Create Graphics Pipeline */
	{
		...
		VkPipelineShaderStageCreateInfo shader_stages[] = { vertex_shader_info, fragment_shader_info, compute_shader_info};
		VkDynamicState dynamic_states[3] = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR, VK_DYNAMIC_STATE_DEPTH_TEST_ENABLE };
		...
		VkComputePipelineCreateInfo	pipeline_info_compute = (VkComputePipelineCreateInfo)
		{
			.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
			.layout = ppln->vk_pipeline_layout_,
			.stage = compute_shader_info
		...
		KSAI_VK_ASSERT(vkCreateComputePipelines(vk_logical_device_, VK_NULL_HANDLE, 1, &pipeline_info_compute, NULL, &ppln->vk_pipeline_compute_));
		...
		vkDestroyShaderModule(vk_logical_device_, vertex_shader_module, NULL);
		vkDestroyShaderModule(vk_logical_device_, fragment_shader_module, NULL);
		vkDestroyShaderModule(vk_logical_device_, compute_shader_module, NULL);
	}
...
void pipeline_vk_destroy3_compute(pipeline_vk *_ppln)
{
	...
	vkDestroyPipelineLayout(vk_logical_device_, _ppln->vk_pipeline_layout_, NULL);
	vkDestroyPipeline(vk_logical_device_, _ppln->vk_pipeline_, NULL);
	vkDestroyPipeline(vk_logical_device_, _ppln->vk_pipeline_compute_, NULL);
}
